...
i_node_result_t create_entry(fs_state_t *state, unsigned char *path, const uint32_t parent_i_node_index,
    ...
    for (size_t i = 0; i < parent_i_node->blocks_used; i++) {
        ...
        for (size_t j = 0; j < MAX_CHILD_ENTRIES_PER_BLOCK; j++) {
            ...
            if (cmp_result == FULL_PATH_EQUAL) {
                add_completion_entry(state, client_id, FS_ERR_ALREADY_EXISTS, 0, 0, SIZE_MAX);
                ...
                if (state->i_node_table[child_entries[j].i_node_index].mode & IS_DIRECTORY_BIT_SET) {
                    if (!valid_permissions(&state->i_node_table[child_entries[j].i_node_index], client_id, PERM_EXECUTE)) {
                        add_completion_entry(state, client_id, FS_ERR_PERMISSION, 0, 0, SIZE_MAX);
                        return (i_node_result_t){-1, FS_ERR_PERMISSION};
                    }
                    return create_entry(state, path, child_entries[j].i_node_index, permissions, client_id,
                                        is_directory);
                } else {
                    add_completion_entry(state, client_id, FS_ERR_INVALID_PATH, 0, 0, SIZE_MAX);
                    return (i_node_result_t){-1, FS_ERR_INVALID_PATH};
    ...
    if (slot_info.return_code != FS_OK) {
        add_completion_entry(state, client_id, slot_info.return_code, 0, 0, SIZE_MAX);
        return (i_node_result_t){-1, slot_info.return_code};
    }
    return add_entry(state, parent_i_node_index, path, permissions, client_id, slot_info.block_index,
                     slot_info.entry_index, is_directory);
}


void delete_entry_operation(fs_state_t *state, const uint8_t client_id, unsigned char *path) {
    i_node_result_t parent_i_node = get_i_node_index(state, path, ROOT_DIRECTORY_I_NODE_INDEX, client_id, GET_PARENT_I_NODE);
    if (parent_i_node.return_code != FS_OK) {
        add_completion_entry(state, client_id, parent_i_node.return_code, 0, 0, SIZE_MAX);
        return;
    }
    ...
    if (i_node_index.return_code != FS_OK) {
        add_completion_entry(state, client_id, i_node_index.return_code, 0, 0, SIZE_MAX);
        return;
    }
    ...
    if (is_i_node_open(state, i_node_index.index)) {
    ...
    if (state->i_node_table[i_node_index.index].mode & IS_DIRECTORY_BIT_SET) {
        ...
        add_completion_entry(state, client_id, res, 0, 0, SIZE_MAX);
    } else {
...
void set_entry_permissions_operation(fs_state_t *state, const uint8_t client_id, const permissions_t permissions,
                                     unsigned char *path) {
    i_node_result_t i_node_index = get_i_node_index(state, path, ROOT_DIRECTORY_I_NODE_INDEX, client_id, GET_TARGET_I_NODE);
    if (i_node_index.return_code != FS_OK) {
        add_completion_entry(state, client_id, i_node_index.return_code, 0, 0, SIZE_MAX);
...
void get_entry_permissions_operation(fs_state_t *state, const uint8_t client_id, unsigned char *path) {
    i_node_result_t i_node_index = get_i_node_index(state, path, ROOT_DIRECTORY_I_NODE_INDEX, client_id, GET_TARGET_I_NODE);
    if (i_node_index.return_code != FS_OK) {
        add_completion_entry(state, client_id, i_node_index.return_code, 0, 0, SIZE_MAX);
        return;
    ...
    add_completion_entry(state, client_id, FS_OK, permissions, 0, SIZE_MAX);
}


void get_entry_size_operation(fs_state_t *state, const uint8_t client_id, unsigned char *path) {
    i_node_result_t i_node_index = get_i_node_index(state, path, ROOT_DIRECTORY_I_NODE_INDEX, client_id, GET_TARGET_I_NODE);
    if (i_node_index.return_code != FS_OK) {
        add_completion_entry(state, client_id, i_node_index.return_code, 0, 0, SIZE_MAX);
        return;
    }
    add_completion_entry(state, client_id, FS_OK, state->i_node_table[i_node_index.index].entry_size, 0, SIZE_MAX);
}


void entry_exists_operation(fs_state_t *state, const uint8_t client_id, unsigned char *path) {
    i_node_result_t i_node_index = get_i_node_index(state, path, ROOT_DIRECTORY_I_NODE_INDEX, client_id, GET_TARGET_I_NODE);
    if (i_node_index.return_code != FS_OK) {
        add_completion_entry(state, client_id, FS_OK, 0, 0, SIZE_MAX);
        return;
    }
    add_completion_entry(state, client_id, FS_OK, 1, 0, SIZE_MAX);
}


void list_directory_operation(fs_state_t *state, const uint8_t client_id, unsigned char *path) {
    ...
    const size_t buffer_index = get_free_buffer(state->clients[client_id].completion_buffer_table);
    if (buffer_index == SIZE_MAX) {
        add_completion_entry(state, client_id, FS_ERROR_NO_FREE_COMPLETION_BUFFERS, 0, 0, SIZE_MAX);
        return;
    }
    uint8_t *client_buffer_data = (uint8_t *)&state->clients[client_id].completion_buffers[buffer_index];
    ...
    add_completion_entry(state, client_id, FS_OK, 0, 0, buffer_index);
}

// ------------------------------ File operation functions ------------------------------- //

void open_file_operation(fs_state_t *state, const uint8_t client_id, const file_open_operations_t requested_operations, unsigned char *path) {
    i_node_result_t i_node_index = get_i_node_index(state, path, ROOT_DIRECTORY_I_NODE_INDEX, client_id, GET_TARGET_I_NODE);
    if (i_node_index.return_code != FS_OK) {
    ...
    if (fd.return_code != FS_OK) {
        ...
        add_completion_entry(state, client_id, fd.return_code, 0, 0, SIZE_MAX);
        return;
    }
    add_completion_entry(state, client_id, FS_OK, fd.file_id, fd.cursor_position, SIZE_MAX);
}


void close_file_operation(fs_state_t *state, const uint8_t client_id, const uint32_t file_descriptor_index) {
...
void read_file_operation(fs_state_t *state, const uint8_t client_id, const uint32_t file_descriptor_index, const size_t length) {
    file_descriptor_result_t fd = get_file_descriptor(state, client_id, file_descriptor_index);
    if (fd.return_code != FS_OK) {
        add_completion_entry(state, client_id, fd.return_code, 0, 0, SIZE_MAX);
        return;
    ...
    const size_t buffer_index = get_free_buffer((bool *)&state->clients[client_id].completion_buffer_table);
    if (buffer_index == SIZE_MAX) {
        add_completion_entry(state, client_id, FS_ERROR_NO_FREE_COMPLETION_BUFFERS, 0, 0, SIZE_MAX);
        return;
    }
    ...
    if (rc != FS_OK) {
        add_completion_entry(state, client_id, rc, 0, 0, buffer_index);
        return;
    }
    add_completion_entry(state, client_id, return_code, fd.descriptor->cursor_position - cursor_before, fd.descriptor->cursor_position, buffer_index);
}


void write_file_operation(fs_state_t *state, const uint8_t client_id, const uint32_t file_descriptor_index, const size_t length,
    ...
    size_t cursor_before = fd.descriptor->cursor_position;
    uint8_t *client_buffer_data = (uint8_t *)&state->clients[client_id].submission_buffers[submission_buffer_index];
    ...
    if (rc != FS_OK) {
        add_completion_entry(state, client_id, rc, 0, 0, SIZE_MAX);
        return;
    }
    add_completion_entry(state, client_id, return_code, fd.descriptor->cursor_position - cursor_before, fd.descriptor->cursor_position, SIZE_MAX);
}


void seek_file_operation(fs_state_t *state, const uint8_t client_id, const uint32_t file_descriptor_index,
    ...
    if (position > state->i_node_table[fd.descriptor->i_node_index].entry_size) {
        add_completion_entry(state, client_id, FS_ERR_OUT_OF_BOUNDS, 0, 0, SIZE_MAX);
        return;
    }
