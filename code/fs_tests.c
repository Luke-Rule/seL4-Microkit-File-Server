...
void expect_equal_to_client_buffer(const unsigned char *expected, size_t length, const char *test_message, int buffer_index) {
    uint8_t *fs_buffer_base = &client_data->completion_buffers[buffer_index].data[0];
...
void run_tests() {
    ...
    rc = get_next_completion_entry(client_data, &rc_fail);
    ...
    rc = send_list_entries_request("/\0", client_data);
    ...
    rc = get_next_completion_entry(client_data, &rc_list);
    microkit_dbg_puts("Submission queue tail after listing entries: ");
    microkit_dbg_put32(client_data->submission_queue_tail);
    microkit_dbg_puts("\n");
    ...
    rc = send_create_file_request("/testfile.txt\0", READ_WRITE_OP, client_data);
    expect_eq_int(rc, FS_OK, "Create file");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_file;
    rc = get_next_completion_entry(client_data, &rc_file);
    ...
    rc = send_write_file_request(0, 0, (const uint8_t *)"an", client_data);
    expect_eq_int(rc, FS_OK, "Queue write zero bytes");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_write0;
    rc = get_next_completion_entry(client_data, &rc_write0);
    ...
    rc = send_read_file_request(0, 0, client_data);
    expect_eq_int(rc, FS_OK, "Queue read zero bytes");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_read0;
    rc = get_next_completion_entry(client_data, &rc_read0);
    ...
    rc = get_next_completion_entry(client_data, &rc_list2);
    expect_eq_int(rc, FS_OK, "Get completion entry for list entries");
    ...
    rc = send_create_file_request("/testfile1.txt\0", READ_WRITE_OP, client_data);
    expect_eq_int(rc, FS_OK, "Queue create second file");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_file2;
    rc = get_next_completion_entry(client_data, &rc_file2);
    ...
    rc = send_write_file_request(1, sizeof(write_data1), write_data1, client_data);
    expect_eq_int(rc, FS_OK, "Queue write to second file");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_write1;
    rc = get_next_completion_entry(client_data, &rc_write1);
    ...
    rc = send_seek_file_request(1, 0, client_data);
    expect_eq_int(rc, FS_OK, "Queue seek start second file");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_seek1;
    rc = get_next_completion_entry(client_data, &rc_seek1);
    ...
    rc = send_read_file_request(1, sizeof(write_data1), client_data);
    expect_eq_int(rc, FS_OK, "Queue read second file");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_read1;
    rc = get_next_completion_entry(client_data, &rc_read1);
    ...
    rc = get_next_completion_entry(client_data, &rc_dup);
    ...
    rc = send_create_directory_request("/testdir\0", READ_WRITE_OP, client_data);
    expect_eq_int(rc, FS_OK, "Queue create directory");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_mkdir;
    rc = get_next_completion_entry(client_data, &rc_mkdir);
    ...
    rc = send_open_file_request(READ_OP, "/testdir\0", client_data);
    expect_eq_int(rc, FS_OK, "Queue open dir as file");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_open_dir_as_file;
    rc = get_next_completion_entry(client_data, &rc_open_dir_as_file);
    ...
    rc = send_create_file_request("/testdir\0", READ_WRITE_OP, client_data);
    expect_eq_int(rc, FS_OK, "Queue create file named dir");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_file_named_dir;
    rc = get_next_completion_entry(client_data, &rc_file_named_dir);
    ...
    rc = get_next_completion_entry(client_data, &rc_list3);
    expect_eq_int(rc, FS_OK, "Get completion entry for list entries");
    ...
    rc = send_create_file_request("/testdir/nestedfile.txt\0", READ_WRITE_OP, client_data);
    expect_eq_int(rc, FS_OK, "Queue create nested file");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_file_nested;
    rc = get_next_completion_entry(client_data, &rc_file_nested);
    ...
    rc = send_write_file_request(2, sizeof(write_data2), write_data2, client_data);
    expect_eq_int(rc, FS_OK, "Queue write to nested file");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_write2;
    rc = get_next_completion_entry(client_data, &rc_write2);
    ...
    rc = send_seek_file_request(2, 0, client_data);
    expect_eq_int(rc, FS_OK, "Queue seek start nested");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_seek2;
    rc = get_next_completion_entry(client_data, &rc_seek2);
    ...
    rc = send_read_file_request(2, sizeof(write_data2), client_data);
    expect_eq_int(rc, FS_OK, "Queue read nested file");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_read2;
    rc = get_next_completion_entry(client_data, &rc_read2);
    ...
    rc = send_list_entries_request("/testdir\0", client_data);
    expect_eq_int(rc, FS_OK, "Queue list entries in directory");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_list_dir1;
    rc = get_next_completion_entry(client_data, &rc_list_dir1);
    ...
    rc = get_next_completion_entry(client_data, &rc_list_root2);
    ...
    rc = send_write_file_request(0, sizeof(write_data), write_data, client_data);
    expect_eq_int(rc, FS_OK, "Queue write to file");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_write0_full;
    rc = get_next_completion_entry(client_data, &rc_write0_full);
    ...
    rc = send_get_entry_size_request("/testfile.txt\0", client_data);
    expect_eq_int(rc, FS_OK, "Queue get size");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_size0;
    rc = get_next_completion_entry(client_data, &rc_size0);
    ...
    rc = send_read_file_request(0, sizeof(write_data), client_data);
    expect_eq_int(rc, FS_OK, "Queue read at end");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_read_oob0;
    rc = get_next_completion_entry(client_data, &rc_read_oob0);
    ...
    rc = send_seek_file_request(0, 1000000, client_data);
    expect_eq_int(rc, FS_OK, "Queue seek beyond end");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_seek_oob0;
    rc = get_next_completion_entry(client_data, &rc_seek_oob0);
    ...
    rc = send_seek_file_request(0, 0, client_data);
    expect_eq_int(rc, FS_OK, "Queue seek start");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_seek_start0;
    rc = get_next_completion_entry(client_data, &rc_seek_start0);
    ...
    rc = get_next_completion_entry(client_data, &rc_read_after_seek0);
    ...
    rc = send_seek_file_request(0, 7, client_data);
    expect_eq_int(rc, FS_OK, "Queue seek middle"); 
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_seek_mid0;
    rc = get_next_completion_entry(client_data, &rc_seek_mid0);
    ...
    rc = send_write_file_request(0, sizeof(more_write_data), more_write_data, client_data);
    expect_eq_int(rc, FS_OK, "Queue write more data");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_write_more0;
    rc = get_next_completion_entry(client_data, &rc_write_more0);
    ...
    rc = get_next_completion_entry(client_data, &rc_seek_start_again0);
    ...
    rc = send_read_file_request(0, rc_write_more0.parameter2, client_data);
    expect_eq_int(rc, FS_OK, "Queue read full file");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_read_full0;
    rc = get_next_completion_entry(client_data, &rc_read_full0);
    ...
    rc = send_seek_file_request(0, rc_read_full0.parameter2, client_data);
    expect_eq_int(rc, FS_OK, "Queue seek exact end");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_seek_exact_end0;
    rc = get_next_completion_entry(client_data, &rc_seek_exact_end0);
    ...
    rc = get_next_completion_entry(client_data, &rc_list_before_delete1);
    ...
    rc = send_delete_entry_request("/testfile1.txt\0", client_data);
    expect_eq_int(rc, FS_OK, "Queue delete '/testfile1.txt'");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_delete_file1;
    rc = get_next_completion_entry(client_data, &rc_delete_file1);
    ...
    rc = get_next_completion_entry(client_data, &rc_list_after_delete1);
    ...
    rc = send_delete_entry_request("/testdir/nestedfile.txt\0", client_data);
    expect_eq_int(rc, FS_OK, "Queue delete nested file");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_delete_nested;
    rc = get_next_completion_entry(client_data, &rc_delete_nested);
    ...
    rc = get_next_completion_entry(client_data, &rc_list_testdir_after_delete);
    ...
    rc = send_get_entry_size_request("/testdir\0", client_data);
    expect_eq_int(rc, FS_OK, "Queue get size of directory");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_size_dir0;
    rc = get_next_completion_entry(client_data, &rc_size_dir0);
    ...
    rc = send_create_file_request("/testdir/nestedfile.txt\0", 0b110, client_data);
    expect_eq_int(rc, FS_OK, "Queue re-create nested file");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_recreate_nested;
    rc = get_next_completion_entry(client_data, &rc_recreate_nested);
    ...
    rc = send_entry_exists_request("/testdir/nestedfile.txt\0", client_data);
    expect_eq_int(rc, FS_OK, "Queue entry exists");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_exists_nested;
    rc = get_next_completion_entry(client_data, &rc_exists_nested);
    ...
    rc = get_next_completion_entry(client_data, &rc_size_dir1);
    ...
    rc = get_next_completion_entry(client_data, &rc_list_before_del_dir);
    ...
    rc = send_delete_entry_request("/testdir\0", client_data);
    expect_eq_int(rc, FS_OK, "Queue delete directory '/testdir'");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_delete_dir;
    rc = get_next_completion_entry(client_data, &rc_delete_dir);
    ...
    rc = get_next_completion_entry(client_data, &rc_list_after_del_dir);
    ...
    rc = send_set_entry_permissions_request("/testdir\0", 0b111, client_data);
    expect_eq_int(rc, FS_OK, "Queue set permissions on deleted dir");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_setperm_deleted_dir;
    rc = get_next_completion_entry(client_data, &rc_setperm_deleted_dir);
    ...
    rc = send_read_file_request(1, sizeof(write_data), client_data);
    expect_eq_int(rc, FS_OK, "Queue read from nested file id 1");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_read_deleted_dir_file;
    rc = get_next_completion_entry(client_data, &rc_read_deleted_dir_file);
    ...
    rc = send_entry_exists_request("/testdir\0", client_data);
    expect_eq_int(rc, FS_OK, "Queue entry exists check");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_exists_dir_deleted;
    rc = get_next_completion_entry(client_data, &rc_exists_dir_deleted);
    ...
    rc = get_next_completion_entry(client_data, &rc_list_deleted_dir);
    ...
    rc = get_next_completion_entry(client_data, &rc_seek_start_again1);
    ...
    rc = send_write_file_request(0, 2791, lots, client_data);
    expect_eq_int(rc, FS_OK, "Queue large write");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_write_lots0;
    rc = get_next_completion_entry(client_data, &rc_write_lots0);
    ...
    rc = get_next_completion_entry(client_data, &rc_seek_start_again2);
    expect_eq_int(rc, FS_OK, "Get completion for seek start again");
    ...
    rc = send_read_file_request(0, rc_write_lots0.parameter2, client_data);
    expect_eq_int(rc, FS_OK, "Queue full file read after large write");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_read_lots0;
    rc = get_next_completion_entry(client_data, &rc_read_lots0);
    ...
    rc = send_close_file_request(0, client_data);
    expect_eq_int(rc, FS_OK, "Queue close file 0");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_close_file0;
    rc = get_next_completion_entry(client_data, &rc_close_file0);
    ...
    rc = get_next_completion_entry(client_data, &rc_close_file0_again);
    ...
    rc = get_next_completion_entry(client_data, &rc_read_closed0);
    ...
    rc = send_set_entry_permissions_request("/testfile.txt\0", PERM_READ, client_data);
    expect_eq_int(rc, FS_OK, "Queue set permissions");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_set_perm_file;
    rc = get_next_completion_entry(client_data, &rc_set_perm_file);
    ...
    rc = send_get_entry_permissions_request("/testfile.txt\0", client_data);
    expect_eq_int(rc, FS_OK, "Queue get permissions");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_get_perm_file;
    rc = get_next_completion_entry(client_data, &rc_get_perm_file);
    ...
    rc = send_open_file_request(READ_OP, "/testfile.txt\0", client_data);
    expect_eq_int(rc, FS_OK, "Queue reopen file");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_reopen_file0;
    rc = get_next_completion_entry(client_data, &rc_reopen_file0);
    ...
    rc = get_next_completion_entry(client_data, &rc_seek_after_reopen0);
    ...
    rc = get_next_completion_entry(client_data, &rc_read_after_reopen0);
    ...
    rc = get_next_completion_entry(client_data, &rc_write_readonly0);
    ...
    rc = send_delete_entry_request("/nonexistent.txt\0", client_data);
    expect_eq_int(rc, FS_OK, "Queue delete non-existent file");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_delete_nonexistent;
    rc = get_next_completion_entry(client_data, &rc_delete_nonexistent);
    ...
    rc = send_create_file_request("//df\0", READ_WRITE_OP, client_data);
    expect_eq_int(rc, FS_OK, "Queue create with invalid name");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_create_invalid5;
    rc = get_next_completion_entry(client_data, &rc_create_invalid5);
    ...
    rc = send_create_file_request("d/f\0", READ_WRITE_OP, client_data);
    expect_eq_int(rc, FS_OK, "Queue create invalid d/f");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_create_invalid6;
    rc = get_next_completion_entry(client_data, &rc_create_invalid6);
    ...
    rc = get_next_completion_entry(client_data, &rc_create_invalid7);
    ...
    rc = get_next_completion_entry(client_data, &rc_create_invalid8);
    ...
    rc = send_create_directory_request("//df\0", READ_WRITE_OP, client_data);
    expect_eq_int(rc, FS_OK, "Queue mkdir //df");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_mkdir_invalid5;
    rc = get_next_completion_entry(client_data, &rc_mkdir_invalid5);
    ...
    rc = send_create_directory_request("d/f\0", READ_WRITE_OP, client_data);
    expect_eq_int(rc, FS_OK, "Queue mkdir d/f");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_mkdir_invalid6;
    rc = get_next_completion_entry(client_data, &rc_mkdir_invalid6);
    ...
    rc = get_next_completion_entry(client_data, &rc_mkdir_invalid7);
    ...
    rc = get_next_completion_entry(client_data, &rc_mkdir_invalid8);
    ...
    rc = send_delete_entry_request("/\0", client_data);
    expect_eq_int(rc, FS_OK, "Queue delete root");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_del_root;
    rc = get_next_completion_entry(client_data, &rc_del_root);
    ...
    rc = send_create_directory_request("/testfile.txt\0", READ_WRITE_OP, client_data);
    expect_eq_int(rc, FS_OK, "Queue mkdir with file name");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_mkdir_file_name;
    rc = get_next_completion_entry(client_data, &rc_mkdir_file_name);
    ...
    rc = send_write_file_request(99, sizeof(write_data), write_data, client_data);
    expect_eq_int(rc, FS_OK, "Queue write to invalid fd");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_write_invalid_fd;
    rc = get_next_completion_entry(client_data, &rc_write_invalid_fd);
    ...
    rc = send_create_file_request("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\0", READ_WRITE_OP, client_data);
    expect_eq_int(rc, FS_OK, "Queue create max length file name");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_create_maxlen_file;
    rc = get_next_completion_entry(client_data, &rc_create_maxlen_file);
    ...
    rc = send_create_directory_request("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\0", READ_WRITE_OP, client_data);
    expect_eq_int(rc, FS_OK, "Queue mkdir max length name");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_mkdir_maxlen_dir;
    rc = get_next_completion_entry(client_data, &rc_mkdir_maxlen_dir);
    ...
    rc = send_create_file_request("/batchedfile.txt\0", READ_WRITE_OP, client_data);
    expect_eq_int(rc, FS_OK, "Queue create batched file");
    notify_file_server(client_data, 1);
    completion_queue_entry_t rc_create_batched;
    rc = get_next_completion_entry(client_data, &rc_create_batched);
    ...
    rc = send_write_file_request(1, sizeof(write_data), write_data, client_data);
    ...
    rc = get_next_completion_entry(client_data, &rc_batched_write);
    ...
    rc = get_next_completion_entry(client_data, &rc_batched_seek);
    ...
    rc = get_next_completion_entry(client_data, &rc_batched_read);
