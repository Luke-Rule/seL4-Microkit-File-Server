...
i_node_result_t add_entry(fs_state_t *state, const uint32_t parent_i_node_index, const unsigned char *name,
                          ...
                          const size_t block_index, const size_t entry_index, const bool is_directory)
{
                             ...
                             const bool is_directory)
{
    ...
    microkit_debug_puts(OUTPUT_VERBOSITY, (char *)client_buffer);
    microkit_debug_puts(OUTPUT_VERBOSITY, "\n");
    if (client_buffer[0] != '/') {
        return (i_node_result_t){-1, FS_ERR_INVALID_PATH};
    }
    ...
    return add_entry(state, parent_i_node_index, client_buffer, permissions, client_id, slot_info.block_index,
