...
microkit_msginfo protected(microkit_channel channel, microkit_msginfo msginfo) {
    ...
    switch (operation) {
        case OP_CREATE_FILE: {
            ...
            const i_node_result_t i_node = create_entry(&fs_state, client_buffer_base(channel),
                                                        ROOT_DIRECTORY_I_NODE_INDEX, permissions, channel, CREATE_FILE);
       
            if (i_node.return_code != FS_OK) {
                return_code = i_node.return_code;
            } else {
                // if creation succeeded, open file for client
        ...
        case OP_CREATE_DIRECTORY: {
            ...
            const i_node_result_t i_node = create_entry(&fs_state, client_buffer_base(channel),
                                                        ROOT_DIRECTORY_I_NODE_INDEX, permissions, channel, CREATE_DIRECTORY);
...
void init(void) {
    ...
    fs_state.blocks = (block_t *)(fs_memory_base + MAX_NUMBER_OF_BLOCKS + sizeof(i_node_t) * MAX_NUMBER_OF_INODES + sizeof(file_descriptor_t)
                      * NUMBER_OF_CLIENTS * MAX_OPEN_FILES_PER_CLIENT);
